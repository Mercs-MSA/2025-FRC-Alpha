...
{
  if(m_elevator.getPosition()-20.0<0.2)
  {
    m_pivot.moveMethod(5.0); //fill in with correct motor pos
    if(m_pivot.getPositionPivot()-5.0<0.2)
    {
      m_claw.setVoltage(5.0);
    }
  }
  ...
  {
    m_elevator.moveMethod((null), false); //L1 pos
    collectCoral();
...
{
  if(m_elevator.getPosition()-30.0<0.2)
  {
    m_pivot.moveMethod(10.0);
    if(m_pivot.getPositionPivot()-10.0<0.2)
    {
      m_claw.setVoltage(5.0);
    }
  }
